...
if ($('#ordersChart').length) {
  var activeOwners = $('#ordersChart').data('active-owners');
  var inactiveOwners = $('#ordersChart').data('inactive-owners');
  ...
  var options2 = {
      ...
      dataLabels: {
          ...
          formatter: function (val) {
              return val; // Display the count on bars
          }
      },
      tooltip: {
          enabled: true,
          y: {
              formatter: function (val) {
                  return val; // Display the count in tooltips
              }
          }
      },
      grid: {
...
// Orders Chart - END
  ...
  // Owners Chart
  if ($('#ownersChart').length) {
    var activeOwnerprop = $('#ownersChart').data('active-ownerprop');
    var inactiveOwnerprop = $('#ownersChart').data('inactive-ownerprop');
    ...
    var options2 = {
        ...
        series: [{
            ...
            data: [activeOwnerprop]
        }, {
            name: 'Inactive Property',
            data: [inactiveOwnerprop]
        }],
        colors: [colors.primary, colors.danger],
        xaxis: {
    ...
    new ApexCharts(document.querySelector("#ownersChart"), options2).render();
  }
  // Orders Chart - END

 // Tenants Chart
 if ($('#tenantsChart').length) {
  var activeTenants = $('#tenantsChart').data('active-tenants');
  var pendingTenants = $('#tenantsChart').data('pending-tenants');
  ...
  new ApexCharts(document.querySelector("#tenantsChart"), options2).render();
}
// Orders Chart - END
  ...
  // Monthly Sales Chart
  if($('#monthlySalesChart').length && monthlyListingsData.length > 0) {
    var options = {
      chart: {
        type: 'bar',
        height: '318',
      ...
      series: [{
        name: 'Listings',
        data: monthlyListingsData.map(item => item.total_listings)
      }],
      xaxis: {
        type: 'category',
        categories: monthlyListingsData.map(item => item.year_month),
        axisBorder: {
          color: colors.gridBorder,
        },
        axisTicks: {
    ...
    options.xaxis.categories = monthlyListingsData.map(item => item.year + '-' + item.month);
