...
class TenantController extends Controller
{
    public function TenantDashboard()
    {
        ...
        return view('frontend.dashboard.edit_profile', compact('tenantInfo')); // Pass the tenant information to the view
    }
    ...
    public function acceptTenant(Request $request, $id)
    {
        $tenant = Tenant::findOrFail($id);
        $tenant->status = 'accepted';
        $tenant->rental_duration = $request->input('duration', 6);

        $tenant->save();

        $duration = $tenant->rental_duration;
        $dueDate = Carbon::now();
        for ($i = 0; $i < $duration; $i++) {
            ...
            $rentBill = new RentBill([
                ...
                'monthly_rent' => $tenant->property_price,
