...
class OwnerController extends Controller
{
    public function OwnerDashboard() {
        ...
        $tenantCount = Tenant::where('owner_email', $ownerEmail)->count();
        // Count active properties
        $activeOwnerProp = Property::where('owner_id', $id)
            ->where('status', '1')
            ->count();

        // Count inactive properties
        $inactiveOwnerProp = Property::where('owner_id', $id)
            ->where('status', '2')
            ->count();
        // Count active tenants
        $activeTenants = Tenant::where('owner_email', $ownerEmail)
            ->where('status', 'active')
            ->count();

        // Count pending tenants (assuming pending status is represented by another status)
        $pendingTenants = Tenant::where('owner_email', $ownerEmail)
            ->where('status', 'pending')
            ->count();
        return view('owner.index', compact('property', 'propertyCount', 'tenantCount', 'activeOwnerProp', 'inactiveOwnerProp', 'activeTenants', 'pendingTenants'));
