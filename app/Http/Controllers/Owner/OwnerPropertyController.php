...
class OwnerPropertyController extends Controller
{
    ...
    public function OwnerTenantmanager()
    {
        ...
        $userPaymentLogs = PaymentLog::where('sender_email', auth()->user()->email)->orWhere('recipient_email', auth()->user()->email)->get();
    ...
    public function paymentLogs()
{
    ...
    $columns = ['id', 'amount', 'recipient_email', 'sender_email', 'description', 'created_at', 'receipt_image_path'];

    // Log the start of the function
    Log::info('Starting paymentLogs function');

    // Fetch all payment logs
    $allPaymentLogs = PaymentLog::all();
    Log::info('Fetched all payment logs', ['count' => $allPaymentLogs->count()]);

    // Fetch logs specific to the user
    $userPaymentLogs = PaymentLog::select($columns)->where('recipient_email', $userEmail)->get();
    Log::info('Fetched user-specific payment logs', ['userEmail' => $userEmail, 'count' => $userPaymentLogs->count()]);

    // Iterate over allPaymentLogs to check for existing receipts
    foreach ($allPaymentLogs as $log) {
        // Check if a receipt exists for the current log
        $receiptExists = Receipt::where('receipt_number', $log->id)->exists();
        // Add a custom attribute to indicate if the receipt exists
        $log->receiptExists = $receiptExists;
        Log::info('Checked receipt existence for all logs', ['log_id' => $log->id, 'receiptExists' => $receiptExists]);
    }

    // Repeat the process for user-specific payment logs
    foreach ($userPaymentLogs as $log) {
        // Check if a receipt exists for the current log
        $receiptExists = Receipt::where('receipt_number', $log->id)->exists();
        // Add a custom attribute to indicate if the receipt exists
        $log->receiptExists = $receiptExists;
        Log::info('Checked receipt existence for user logs', ['log_id' => $log->id, 'receiptExists' => $receiptExists]);
    }

    // Log the end of the function
    Log::info('Finished paymentLogs function');
    ...
    //Utilities View table function tenant
    public function utilities()
{
    if (Auth::check()) {
        $userEmail = Auth::user()->email;

        // Filter the billings based on the logged-in user's email
        $rentBillings = RentBill::where('tenant_email', $userEmail)->get();

        return view('tenant.utilities', compact('rentBillings'));
    } else {
